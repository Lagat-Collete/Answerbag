...
function myButton() {
  ...
  if (currentAnswer1 == "substr()") {
    score += 10
  };
  if (currentAnswer2 == "document.myform.elements.length") {
    score += 10
  };
  if (currentAnswer3 == "whether there is javascript or not") {
    score += 10
  };
  if (currentAnswer4 == "display the content of the tags <noscript>") {
    score += 10
  };
  if (currentAnswer5 == "/* var x = 0; */ var y = 0;") {
    score += 10
  };
  if (currentAnswer6 == "onmouseout") {
    score += 10
  };
  if (currentAnswer7 == "onmousescroller") {
    score += 10
  };
  if (currentAnswer8 == "Both A and B are true.") {
    score += 10
  };
  if (currentAnswer9 == "charAt()") {
    score += 10
  };
  if (currentAnswer10 == "sum()") {
    score += 10
  };
  ...
  if (score > 79) {
    messageText.textContent = "You have excellently passed."

  } else if (score > 49 && score < 80) {
    messageText.textContent = " You have fairly passed."

  } else {
    messageText.textContent = "you have scored poorly and they need to retake the test."
  }
  onClick()
}